...
namespace H22\Widget\Brand;

class Brand extends \Municipio\Widget\Source\WidgetTemplate
{
    ...
    public function viewController($args, $instance)
    {
        ...
        if ($logo = $this->get_field('widget_header_logotype')) {
            ...
            if ($link = $this->get_field('widget_header_url')) {
                $this->data['link'] = $link;
                $this->data['link_title'] = $link['title'];
                $this->data['link_url'] = $link['url'];
                $this->data['link_target'] = $link['target'];
            }
        }
    }
